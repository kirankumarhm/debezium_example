...
- ✅ Uses Debezium's native MongoDB Sink Connector with `MongoDbCdcHandler`
...
Apache Kafka
    ↓ Event Streaming
Debezium MongoDB Sink Connector (MongoDbCdcHandler)
    ↓ Write Operations
...
- **Apache Kafka**: Message broker for CDC event streaming (KRaft mode, no Zookeeper)
...
| `tombstones.on.delete` | `false` | Prevents tombstone records that crash the MongoDB CDC handler |
| `schemas.enable` | `true` | Required for the MongoDB CDC handler to parse `before`/`after` fields |
| **NO `transforms`** | — | Preserves full CDC envelope with `before` and `after` |

**Why no `transforms` (unwrap)?**

The `ExtractNewRecordState` transform flattens the CDC envelope, removing the `before`/`after` structure. The Debezium MongoDB Sink Connector with `MongoDbCdcHandler` **requires the full CDC envelope** to properly handle all operation types (INSERT/UPDATE/DELETE).
...
### UPDATE Event (`op: "u"`) — requires REPLICA IDENTITY FULL
...
### DELETE Event (`op: "d"`) — requires REPLICA IDENTITY FULL
...
> **Note:** Without `REPLICA IDENTITY FULL`, the `before` field in UPDATE events will be `null`, and in DELETE events it will only contain the primary key (`id`) with all other fields as `null`.
...
# Verify in MongoDB (wait 2-3 seconds for propagation)
sleep 3
docker exec mongodb mongosh cmpolicydb --eval "db.customers.find({first_name: 'Alice'}).pretty()"
```
...
sleep 3
docker exec mongodb mongosh cmpolicydb --eval "db.customers.countDocuments({first_name: 'Alice'})"
# Should return: 0
```
...
# MongoDB count
docker exec mongodb mongosh cmpolicydb --eval "db.customers.countDocuments()"
...
**Root Cause:** By default, Debezium PostgreSQL connector sends **tombstone records** (null value, null schema) after DELETE events, which causes the MongoDB CDC handler to crash with:
```
...
1. Set `tombstones.on.delete: false` in `postgres-source.json` (already configured)
2. Ensure MongoDB sink has proper DELETE configuration (already configured):
   - `change.data.capture.handler`: `MongoDbCdcHandler`
   - `delete.enabled`: `true`
   - `delete.on.null.values`: `true`

3. If connectors need restart:
```bash
...
| Option | Behavior |
|--------|----------|
| `DEFAULT` | Only primary key in `before` field (other fields are null) |
| `FULL` | All columns included in `before` field |
| `INDEX` | Uses a specific unique index |
| `NOTHING` | No `before` data captured |
...
# Consume messages from topic to verify data flows
...
### Why `tombstones.on.delete: false`?

By default, Debezium sends **two messages** for each DELETE:
1. **DELETE event** — contains `before` field with deleted data (`op: "d"`)
2. **Tombstone record** — a null message for Kafka log compaction

The tombstone has both null value and null schema, causing the MongoDB CDC handler to crash. Setting `tombstones.on.delete: false` prevents tombstone generation. The DELETE event itself is sufficient for MongoDB to process the deletion.

### Why `schemas.enable: true`?

The Debezium MongoDB Sink Connector with `MongoDbCdcHandler` requires schema information to parse the CDC envelope structure (`before`, `after`, `op` fields). Without schemas, the handler cannot determine field types and operation semantics.
...
**Performance Impact:** Minimal for most workloads. The additional WAL data is already in memory during the transaction.
...
- [Debezium MongoDB Sink Connector](https://debezium.io/documentation/reference/stable/connectors/mongodb-sink.html)
